...
func _add_status_ui(data):
	...
	var anim_id = AnimationController.play_animation(idle_anim, pos, Vector2(0.5, 0.5), Color.WHITE, 0.0, flip, 60, true)
...
func _update_hand_ui(dc):
	_update_deck_ui(dc) # 手札更新のタイミングで山札も更新
	...
	for i in range(4):
		...
		var card = dc.hand[i]
		if card != null:
			btn.text = card.name # デバッグ用に文字も残す
			btn.set_meta("action_data", card)
			btn.set_meta("hand_idx", i)
			btn.visible = true
			
			# フレームの表示 (TextureRect使用)
			var frame = TextureRect.new()
			frame.texture = load(ImageList.DATA["card_frame"][card.frame])
			frame.position = Vector2.ZERO+Vector2(-20,-20) # ボタン内にぴったり合わせる
			frame.scale = Vector2(0.4,0.4) # ボタン内にぴったり合わせる
			btn.add_child(frame)
		else:
			btn.visible = false
